...
def client_commande_add():
    ...
    sql_commande = "INSERT INTO commande (utilisateur_id, etat_id, date_achat) VALUES (%s, 1, %s)"
    mycursor.execute(sql_commande, (id_client, datetime.now()))
    id_commande = mycursor.lastrowid

    for ligne in lignes_panier:
        sql_prix = "SELECT prix_declinaison FROM declinaison_meuble WHERE id_declinaison_meuble = %s"
        mycursor.execute(sql_prix, (ligne['meuble_declinaison_id'],))
        prix_unitaire = mycursor.fetchone()['prix_declinaison']

        sql_insert_ligne = "INSERT INTO ligne_commande (commande_id, meuble_declinaison_id, prix, quantite) VALUES (%s, %s, %s, %s)"
        mycursor.execute(sql_insert_ligne, (id_commande, ligne['meuble_declinaison_id'], prix_unitaire, ligne['quantite']))

    sql_delete_panier = "DELETE FROM ligne_panier WHERE utilisateur_id = %s"
    mycursor.execute(sql_delete_panier, (id_client,))

    get_db().commit()
    flash(u'Votre commande a été validée avec succès ! (sans adresse)', 'alert-success')
