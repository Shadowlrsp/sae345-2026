...
def client_article_show():
    ...
    sql = '''SELECT m.id_meuble AS id_article,
           ...
           MIN(dm.id_declinaison_meuble) AS id_declinaison_unique,
           (SELECT COUNT(*) FROM commentaire c WHERE c.meuble_id = m.id_meuble) AS nb_avis,
           (SELECT COUNT(*) FROM note n WHERE n.meuble_id = m.id_meuble) AS nb_notes,
           (SELECT AVG(note) FROM note n WHERE n.meuble_id = m.id_meuble) AS moy_notes
           FROM meuble m
           LEFT JOIN materiau ma ON m.materiau_id = ma.id_materiau
           LEFT JOIN type_meuble t ON m.type_meuble_id = t.id_type_meuble
           JOIN declinaison_meuble dm ON m.id_meuble = dm.meuble_id AND dm.actif = 1
    '''
    
    list_param = []
    
    condition_and = [" m.actif = 1 "]
